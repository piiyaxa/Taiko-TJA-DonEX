...
//			this.On非活性化();
		...
		public override void On活性化()
		{
            ...
            {
                this.pfMusicName = new CPrivateFastFont(new FontFamily(TJAPlayer3.ConfigIni.FontName), 20);
                this.pfSubtitle = new CPrivateFastFont(new FontFamily(TJAPlayer3.ConfigIni.FontName), 12);
            }
            else
            {
                this.pfMusicName = new CPrivateFastFont(new FontFamily("MS UI Gothic"), 20);
                this.pfSubtitle = new CPrivateFastFont(new FontFamily("MS UI Gothic"), 12);
            }

			if (!string.IsNullOrEmpty(TJAPlayer3.ConfigIni.FontName))
			{
				this.pfMusicName2 = new CPrivateFastFont(new FontFamily(TJAPlayer3.ConfigIni.FontName), 25);
				this.pfSubtitle2 = new CPrivateFastFont(new FontFamily(TJAPlayer3.ConfigIni.FontName), 17);
			}
			else
			{
				this.pfMusicName2 = new CPrivateFastFont(new FontFamily("MS UI Gothic"), 25);
				this.pfSubtitle2 = new CPrivateFastFont(new FontFamily("MS UI Gothic"), 17);
			}
		...
		public override void On非活性化()
		{
			...
			TJAPlayer3.t安全にDisposeする(ref pfMusicName2);
			TJAPlayer3.t安全にDisposeする(ref pfSubtitle2);
		...
		public override void OnManagedリソースの作成()
		{
			...
			CenterCounter = new CCounter();
			CenterCounter2 = new CCounter();
			CenterCounter3 = new CCounter();

			BarEaseOut = new EaseOut(0, 500, 5);
			BarCounterX = new CCounter();
			BarCounterX2 = new CCounter();
		...
		public override int On進行描画()
		{
			...
			else
			{
				...
				while( ( n現在時刻 - this.nスクロールタイマ ) >= nアニメ間隔 )
				{
					...
					else if( this.n現在のスクロールカウンタ <= -100 )
					{
						...
						this.ttk選択している曲の曲名2 = null;
						this.ttk選択している曲のサブタイトル2 = null;
                    ...
                    if(this.b選択曲が変更された && n現在のスクロールカウンタ==0)
                    {
						...
						if (this.ttk選択している曲のサブタイトル2 != null)
						{
							this.ttk選択している曲のサブタイトル2 = null;
							this.b選択曲が変更された = false;
						}
			...
			if( !this.b登場アニメ全部完了 )
			{
                if( this.n現在のスクロールカウンタ == 0 )
                {
					...
					if (CenterCounter.n現在の値 > 24)
					{

						CenterCounter2.t開始(0, 60, 5, TJAPlayer3.Timer);
						CenterCounter.t停止();
						CenterCounter.n現在の値 = 0;
					}

					for (int i = 0; i < 13; i++)
					{
						int nパネル番号C = (((this.n現在の選択行 + 1) + i) + 13) % 13;
						Bar_Center(448 + 425 + BarCounterX.n現在の値 * 2 - BarCounterX2.n現在の値 * 2, TJAPlayer3.Skin.SongSelect_Overall_Y + 128 + 70, this.stバー情報[nパネル番号C].strジャンル, CenterCounter2.n現在の値);
					}
					switch (r現在選択中の曲.eノード種別)
                    {
                        case C曲リストノード.Eノード種別.SCORE:
                            {
         //                       if (TJAPlayer3.Tx.SongSelect_Frame_Score_5 != null)
         //                       {
									//// 難易度がTower、Danではない
									//if (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Tower && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Dan)
									//{
									//	TJAPlayer3.Tx.SongSelect_Frame_Score_5.t2D描画(TJAPlayer3.app.Device, 318, 275);
									//}
         //                       }
                                #region[ 星 ]
				...
				for( int i = 0; i < 13; i++ )	// パネルは全13枚。
				{
					if( this.ct登場アニメ用[ i ].n現在の値 >= 0 )
					{
						...
						if ( i == 5 )
						{
							...
							if (this.stバー情報[nパネル番号].strタイトル文字列 != "" && this.ttk選択している曲の曲名2 == null)
								this.ttk選択している曲の曲名2 = this.ttk曲名テクスチャを生成する2(this.stバー情報[nパネル番号].strタイトル文字列, Color.White, Color.Black);
							if (this.stバー情報[nパネル番号].strサブタイトル != "" && this.ttk選択している曲のサブタイトル2 == null)
								this.ttk選択している曲のサブタイトル2 = this.ttkサブタイトルテクスチャを生成する2(this.stバー情報[nパネル番号].strサブタイトル);
							switch (r現在選択中の曲.eノード種別)
							{
								case C曲リストノード.Eノード種別.SCORE:
									...
									if (this.ttk選択している曲のサブタイトル2 != null)
									{
										var tx選択している曲のサブタイトル2 = ResolveTitleTexture(ttk選択している曲のサブタイトル2);
										tx選択している曲のサブタイトル2.t2D中心基準描画(TJAPlayer3.app.Device, 425 + (int)(BarCounterX.n現在の値 / 3.3 - BarCounterX2.n現在の値 / 3.3) * 2, -300 + (int)(CenterCounter3.n現在の値 * 8) + (int)(BarCounterX.n現在の値 / 16 - BarCounterX2.n現在の値 / 16) * 2);

										if (this.ttk選択している曲の曲名2 != null)
										{
											ResolveTitleTexture(this.ttk選択している曲の曲名2).t2D中心基準描画(TJAPlayer3.app.Device, 425 + (int)(BarCounterX.n現在の値 / 3.3 - BarCounterX2.n現在の値 / 3.3) * 2, -350 + (int)(CenterCounter3.n現在の値 * 8) + (int)(BarCounterX.n現在の値 / 16 - BarCounterX2.n現在の値 / 16) * 2);
										}
									}
									else
									{
							...
							if (this.ttk選択している曲のサブタイトル != null)
							{
								var tx選択している曲のサブタイトル = ResolveTitleTexture(ttk選択している曲のサブタイトル);
								int nサブタイY = (int)(TJAPlayer3.Skin.SongSelect_Overall_Y + 440 - (tx選択している曲のサブタイトル.sz画像サイズ.Height * tx選択している曲のサブタイトル.vc拡大縮小倍率.Y));
								tx選択している曲のサブタイトル.Opacity = CenterCounter2.n現在の値 * 4;
								tx選択している曲のサブタイトル.t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Overall_Y + 940 + BarCounterX.n現在の値 * 2 - BarCounterX2.n現在の値 * 2, TJAPlayer3.Skin.SongSelect_Overall_Y + 128 + 115 + (int)(CenterCounter2.n現在の値 / 2.5));
								if (this.ttk選択している曲の曲名 != null)
								{
									ResolveTitleTexture(this.ttk選択している曲の曲名).t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Overall_Y + 940 + BarCounterX.n現在の値 * 2 - BarCounterX2.n現在の値 * 2, TJAPlayer3.Skin.SongSelect_Overall_Y + 128 + 115 - (int)(CenterCounter2.n現在の値 / 2.5) + 5);
								}
							}
							else
							{
								if (this.ttk選択している曲の曲名 != null)
								{
									ResolveTitleTexture(this.ttk選択している曲の曲名).t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Overall_Y + 940 + BarCounterX.n現在の値 * 2 - BarCounterX2.n現在の値 * 2, TJAPlayer3.Skin.SongSelect_Overall_Y + 128 + 115 + 5);
								}
							}
							//-----------------
							#endregion
						...
						else
						{
							...
							//-----------------
							if (n現在のスクロールカウンタ != 0)
								this.tジャンル別選択されていない曲バーの描画(TJAPlayer3.Skin.SongSelect_Overall_Y + 750 + BarCounterX.n現在の値 * 2 - BarCounterX2.n現在の値 * 2, xAnime, this.stバー情報[nパネル番号].strジャンル);
							else if (n見た目の行番号 != 5)
								this.tジャンル別選択されていない曲バーの描画(TJAPlayer3.Skin.SongSelect_Overall_Y + 750 + BarCounterX.n現在の値 * 2 - BarCounterX2.n現在の値 * 2, xAnime, this.stバー情報[nパネル番号].strジャンル);
							...
							if (n現在のスクロールカウンタ != 0)
								ResolveTitleTexture(this.stバー情報[nパネル番号].ttkタイトル).t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Overall_Y + 940 + BarCounterX.n現在の値 * 2 - BarCounterX2.n現在の値 * 2, xAnime + 45);
							else if (n見た目の行番号 != 5)
								ResolveTitleTexture(this.stバー情報[nパネル番号].ttkタイトル).t2D中心基準描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Overall_Y + 940 + BarCounterX.n現在の値 * 2 - BarCounterX2.n現在の値 * 2, xAnime + 45);
							#endregion

							bool DanJudge = false;
							if (TJAPlayer3.Tx.Crown_t != null /*&& TJAPlayer3.Tx.DanC_Crown_t != null*/ && i != 5 && this.stバー情報[nパネル番号].eバー種別 == Eバー種別.Score)
							{
								...
								for (int j = 4; j >= 0; j--)
								{
									...
									if (TJAPlayer3.Tx.Crown_t[this.stバー情報[nパネル番号].n王冠[j]] != null)
									{
										TJAPlayer3.Tx.Crown_t[this.stバー情報[nパネル番号].n王冠[j]].vc拡大縮小倍率.X = 50f / TJAPlayer3.Tx.Crown_t[this.stバー情報[nパネル番号].n王冠[j]].szテクスチャサイズ.Width;
										TJAPlayer3.Tx.Crown_t[this.stバー情報[nパネル番号].n王冠[j]].vc拡大縮小倍率.Y = 50f / TJAPlayer3.Tx.Crown_t[this.stバー情報[nパネル番号].n王冠[j]].szテクスチャサイズ.Height;
										if (this.stバー情報[nパネル番号].n王冠[j] != 0)
										{
											TJAPlayer3.Tx.Crown_t[this.stバー情報[nパネル番号].n王冠[j]].t2D描画(TJAPlayer3.app.Device, this.ptバーの座標[i].X + 25, TJAPlayer3.Skin.SongSelect_Overall_Y - 23);
			...
			else
			{
				...
				for( int i = 0; i < 13; i++ )	// パネルは全13枚。
				{
...

					{
						...
						bool DanJudge = false;
						if (TJAPlayer3.Tx.Crown_t != null /*&& TJAPlayer3.Tx.DanC_Crown_t != null*/ && n見た目の行番号 != 5 && this.stバー情報[nパネル番号].eバー種別 == Eバー種別.Score)
						{
							/*if (this.stバー情報[nパネル番号].n王冠[(int)Difficulty.Dan] != 0)
							{
								TJAPlayer3.Tx.DanC_Crown_t.vc拡大縮小倍率 = new Vector3(0.75f);
							...
							for (int j = 4; j >= 0; j--)
							{
								...
								if (TJAPlayer3.Tx.Crown_t[this.stバー情報[nパネル番号].n王冠[j]] != null)
								{
									...
									if (this.stバー情報[nパネル番号].n王冠[j] != 0)
									{
										TJAPlayer3.Tx.Crown_t[this.stバー情報[nパネル番号].n王冠[j]].t2D描画(TJAPlayer3.app.Device, TJAPlayer3.Skin.SongSelect_Overall_Y + 750 + BarCounterX.n現在の値 * 2 - BarCounterX2.n現在の値 * 2, xAnime);
										/*if (TJAPlayer3.Tx.Difficulty_Icons != null)
										{
											TJAPlayer3.Tx.Difficulty_Icons.vc拡大縮小倍率.X = 0.4f;
											TJAPlayer3.Tx.Difficulty_Icons.vc拡大縮小倍率.Y = 0.4f;
				...
				if ( this.n現在のスクロールカウンタ == 0 )
                {
					...
					//   TJAPlayer3.Tx.SongSelect_Bar_Center.t2D描画( TJAPlayer3.app.Device, 448 + 425, TJAPlayer3.Skin.SongSelect_Overall_Y + 128);
                        ...
                        case C曲リストノード.Eノード種別.SCORE:
                            {
								...
								//if (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Tower && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Dan)
								//{
        //                            if (TJAPlayer3.stage選曲.r現在選択中の曲.nスコア数 == (int)Difficulty.Edit)
        //                            {
        //                                TJAPlayer3.Tx.SongSelect_Frame_Score_4?.t2D描画(TJAPlayer3.app.Device, 391, 282);
        //                            }
        //                            if (TJAPlayer3.stage選曲.r現在選択中の曲.nスコア数 != (int)Difficulty.Edit)
        //                            {
        //                                TJAPlayer3.Tx.SongSelect_Frame_Score_5?.t2D描画(TJAPlayer3.app.Device, 333, 282);
        //                            }
        //                        }
								#region[ 星 ]
								if (TJAPlayer3.Tx.SongSelect_Level != null)
								...
								if (TJAPlayer3.Tx.Crown_t != null /*&& TJAPlayer3.Tx.DanC_Crown_t != null*/)
								{
									if (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 <= 4)
									{
										for (int i = 0; i < 4; i++)
										{
											if (TJAPlayer3.Tx.Crown_t[r現在選択中のスコア.譜面情報.n王冠[i]] != null)
											{
												TJAPlayer3.Tx.Crown_t[r現在選択中のスコア.譜面情報.n王冠[i]].vc拡大縮小倍率.X = 25f / TJAPlayer3.Tx.Crown_t[r現在選択中のスコア.譜面情報.n王冠[i]].szテクスチャサイズ.Width;
												TJAPlayer3.Tx.Crown_t[r現在選択中のスコア.譜面情報.n王冠[i]].vc拡大縮小倍率.Y = 25f / TJAPlayer3.Tx.Crown_t[r現在選択中のスコア.譜面情報.n王冠[i]].szテクスチャサイズ.Height;
												if (TJAPlayer3.Tx.Crown_t != null && this.r現在選択中のスコア.譜面情報.n王冠[i] >= 0 && this.r現在選択中のスコア.譜面情報.n王冠[i] <= 3)
												{
													...
													{
														TJAPlayer3.Tx.Crown_t[r現在選択中のスコア.譜面情報.n王冠[i]].t2D描画(TJAPlayer3.app.Device, i * 60 + 482, TJAPlayer3.Skin.SongSelect_Overall_Y + 85);
													}
													else if (this.r現在選択中のスコア != null)
													{
														TJAPlayer3.Tx.Crown_t[r現在選択中のスコア.譜面情報.n王冠[i]].t2D描画(TJAPlayer3.app.Device, i * 60 + 482, TJAPlayer3.Skin.SongSelect_Overall_Y + 85);
													}
												}
									...
									{
										TJAPlayer3.Tx.Crown_t[this.r現在選択中のスコア.譜面情報.n王冠[5]].t2D描画(TJAPlayer3.app.Device, 482, TJAPlayer3.Skin.SongSelect_Overall_Y + 57);
									}
									else
									{/*
								...
								#endregion
								#region 選択カーソル
								if (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Tower && TJAPlayer3.stage選曲.n現在選択中の曲の難易度 != (int)Difficulty.Dan && TJAPlayer3.stage選曲.難易度選択切り替え)
                                {
									if (TJAPlayer3.stage選曲.r現在選択中の曲.nスコア数 == (int)Difficulty.Edit)
									{
										TJAPlayer3.Tx.SongSelect_Score_Select?.t2D描画(TJAPlayer3.app.Device, 421 + (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 * 117), 253);
									}
									if (TJAPlayer3.stage選曲.r現在選択中の曲.nスコア数 != (int)Difficulty.Edit)
									{
										TJAPlayer3.Tx.SongSelect_Score_Select?.t2D描画(TJAPlayer3.app.Device, 363 + (TJAPlayer3.stage選曲.n現在選択中の曲の難易度 * 117), 253);
									}
                                }
                                #endregion
                            }
						...
						//(448 + 425, TJAPlayer3.Skin.SongSelect_Overall_Y + 128+ 70
						case C曲リストノード.Eノード種別.BACKBOX:
							if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[11] != null)
							{
								TJAPlayer3.Tx.SongSelect_Bar_Genre2[11].vc拡大縮小倍率.Y = CenterCounter2.n現在の値 * 2 + 3;
								TJAPlayer3.Tx.SongSelect_Bar_Genre2[11].t2D描画(TJAPlayer3.app.Device, 448 + 425, 7 + TJAPlayer3.Skin.SongSelect_Overall_Y + 128 + 70 + 31 - CenterCounter2.n現在の値);
							}
							if (TJAPlayer3.Tx.SongSelect_Bar_Genre[11] != null)
							{
								TJAPlayer3.Tx.SongSelect_Bar_Genre[11].t2D描画(TJAPlayer3.app.Device, 448 + 425, 7 + TJAPlayer3.Skin.SongSelect_Overall_Y + 128 + 70 - CenterCounter2.n現在の値, new Rectangle(0, 0, 391, 31));
								TJAPlayer3.Tx.SongSelect_Bar_Genre[11].t2D描画(TJAPlayer3.app.Device, 448 + 425, 7 + TJAPlayer3.Skin.SongSelect_Overall_Y + 128 + 70 + CenterCounter2.n現在の値 + 33, new Rectangle(0, 33, 391, 31));
							}
                ...
                for( int i = 0; i < 13; i++ )	// パネルは全13枚。
				{
                    ...
                    if( ( i == 5 ) && ( this.n現在のスクロールカウンタ == 0 ) )
					{
						...
						//425, -380 + (int)(CenterCounter3.n現在の値 * 8)
		...
		private CPrivateFastFont pfMusicName2;
		private CPrivateFastFont pfSubtitle2;
		...
		private Point[] ptバーの座標 = new Point[]
		 { new Point( -65, 180 ), new Point( 0, 180 ), new Point( 65, 180 ), new Point( 130, 180 ), new Point( 195, 180 ),
		  new Point( 321, 180 ),
		  new Point( 460, 180 ), new Point( 525, 180 ), new Point( 590, 180 ), new Point( 655, 180 ), new Point( 730, 180 ), new Point( 730, 180 ), new Point( 730, 180 ) };
  ...
  //      private CTexture txバー中央;
		...
		private TitleTextureKey ttk選択している曲の曲名2;
		private TitleTextureKey ttk選択している曲のサブタイトル2;
		...
		public CCounter CenterCounter;
		public CCounter CenterCounter2;
		public CCounter CenterCounter3;

		public EaseOut BarEaseOut;

		public CCounter BarCounterX;
		public CCounter BarCounterX2;

		private void Bar_Center(int x,int y,string strジャンル,int anime)
		{
			int a = 7;
			var CenterGenreIndex = CStrジャンルtoNum.ForGenreBackIndex(strジャンル);
			...
			switch (r現在選択中の曲.eノード種別)
			{
                case C曲リストノード.Eノード種別.SCORE:
					...
					TJAPlayer3.Tx.Bar_Center[CenterGenreIndex].t2D描画(TJAPlayer3.app.Device, 20 + (int)(BarCounterX.n現在の値 / 3.3 - BarCounterX2.n現在の値 / 3.3) * 2, -500 + (int)(CenterCounter3.n現在の値 * 8.5) + (int)(BarCounterX.n現在の値 / 16 - BarCounterX2.n現在の値 / 16) * 2);
					if (TJAPlayer3.stage選曲.r現在選択中の曲.nスコア数 == (int)Difficulty.Edit)
                    {
						TJAPlayer3.Tx.SongSelect_Frame_Score_4?.t2D描画(TJAPlayer3.app.Device, 179 + (int)(BarCounterX.n現在の値 / 3.3 - BarCounterX2.n現在の値 / 3.3) * 2, -271 + (int)(CenterCounter3.n現在の値 * 8.5) + (int)(BarCounterX.n現在の値 / 16 - BarCounterX2.n現在の値 / 16) * 2);
					}
					if (TJAPlayer3.stage選曲.r現在選択中の曲.nスコア数 != (int)Difficulty.Edit)
					{
						TJAPlayer3.Tx.SongSelect_Frame_Score_5?.t2D描画(TJAPlayer3.app.Device, 121 + (int)(BarCounterX.n現在の値 / 3.3 - BarCounterX2.n現在の値 / 3.3) * 2, -271 + (int)(CenterCounter3.n現在の値 * 8.5) + (int)(BarCounterX.n現在の値 / 16 - BarCounterX2.n現在の値 / 16) * 2);
					}
					break;
					...
					for (int i = 0; i < 13; i++)
					{
						...
						if ((i == 5) && (this.n現在のスクロールカウンタ == 0))
						{
							...
							var 曲名TItle = ResolveTitleTexture2(this.ttk選択している曲の曲名2);

							var 曲名SubTItle = ResolveTitleTexture(this.ttk選択している曲のサブタイトル2);

							if (this.ttk選択している曲の曲名2 != null)
							{

								曲名TItle.t2D中心基準描画(TJAPlayer3.app.Device, 425, -380 + (int)(CenterCounter3.n現在の値 * 8));

							}
							
							if(this.ttk選択している曲のサブタイトル2 != null)
							{
								曲名SubTItle.t2D中心基準描画(TJAPlayer3.app.Device, 425, -280 + (int)(CenterCounter3.n現在の値 * 8.5));
							}
			...
			switch (strジャンル)
			{
				case "J-POP":
					#region [ J-POP ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[1] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[1].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[1].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[1] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[1].t2D描画(TJAPlayer3.app.Device, x,a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[1].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				case "アニメ":
					#region [ アニメ ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[2] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[2].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[2].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[2] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[2].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[2].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				case "ゲームミュージック":
					#region [ ゲーム ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[3] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[3].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[3].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[3] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[3].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[3].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				case "ナムコオリジナル":
					#region [ ナムコオリジナル ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[4] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[4].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[4].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[4] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[4].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[4].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				case "クラシック":
					#region [ クラシック ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[5] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[5].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[5].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[5] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[5].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[5].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				case "バラエティ":
					#region [ バラエティ ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[6] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[6].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[6].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[6] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[6].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[6].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				case "どうよう":
					#region [ どうよう ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[7] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[7].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[7].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[7] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[7].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[7].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				case "VOCALOID":
					#region [ ボカロ ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[8] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[8].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[8].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[8] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[8].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[8].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				case "難易度ソート":
					...
					{
						this.tx曲バー_難易度[this.n現在選択中の曲の現在の難易度レベル].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						this.tx曲バー_難易度[this.n現在選択中の曲の現在の難易度レベル].t2D描画(TJAPlayer3.app.Device, x, a + y - anime + 33, new Rectangle(0, 330, 391, 31));
				...
				case "段位道場":
					#region [ 段位道場 ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[9] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[9].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[9].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[9] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[9].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[9].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				case "創作譜面":
					#region [ 創作譜面 ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[10] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[10].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[10].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[10] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[10].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[10].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
				...
				default:
					#region [ その他の場合 ]
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre2[0] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[0].vc拡大縮小倍率.Y = anime * 2 + 3;
						TJAPlayer3.Tx.SongSelect_Bar_Genre2[0].t2D描画(TJAPlayer3.app.Device, x, a + y + 31 - anime);
					}
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[0] != null)
					{
						TJAPlayer3.Tx.SongSelect_Bar_Genre[0].t2D描画(TJAPlayer3.app.Device, x, a + y - anime, new Rectangle(0, 0, 391, 31));
						TJAPlayer3.Tx.SongSelect_Bar_Genre[0].t2D描画(TJAPlayer3.app.Device, x, a + y + anime + 33, new Rectangle(0, 33, 391, 31));
					}
		...
		private void tジャンル別選択されていない曲バーの描画( int x, int y, string strジャンル )
		{
			...
			switch( strジャンル )
            {
				...
				case "段位道場":
					...
					//-----------------
					if (TJAPlayer3.Tx.SongSelect_Bar_Genre[9] != null)
						TJAPlayer3.Tx.SongSelect_Bar_Genre[9].t2D描画(TJAPlayer3.app.Device, x, y);
					//-----------------
					#endregion
					break;
				case "創作譜面":
		...
		private TitleTextureKey ttk曲名テクスチャを生成する2(string str文字, Color forecolor, Color backcolor)
		{
			return new TitleTextureKey(str文字, pfMusicName2, forecolor, backcolor, 410);
		}

		private TitleTextureKey ttkサブタイトルテクスチャを生成する2(string str文字)
		{
			return new TitleTextureKey(str文字, pfSubtitle2, Color.White, Color.Black, 390);
		}
		...
		private CTexture ResolveTitleTexture2(TitleTextureKey titleTextureKey)
		{
			if (!_titleTextures.TryGetValue(titleTextureKey, out var texture))
			{
				texture = GenerateTitleTexture(titleTextureKey);
	    ...
	    {
	        using (var bmp = new Bitmap(titleTextureKey.cPrivateFastFont.DrawPrivateFont( titleTextureKey.str文字, titleTextureKey.forecolor, titleTextureKey.backcolor)))
	        {
